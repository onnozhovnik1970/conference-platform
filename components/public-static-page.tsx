...
const staticPageMarkdownComponents: Partial<Components> = {
  p: ({ children }) => <p className="mb-4 text-sm leading-relaxed text-slate-600 last:mb-0 md:text-base">{children}</p>,
  strong: ({ children }) => <strong className="font-semibold text-[#0F172A]">{children}</strong>,
  em: ({ children }) => <em className="italic text-slate-700">{children}</em>,
  h1: ({ children }) => (
    <h2 className="mb-3 mt-8 scroll-mt-20 text-2xl font-bold tracking-tight text-[#0F172A] first:mt-0">{children}</h2>
  ),
  h2: ({ children }) => (
    <h3 className="mb-2 mt-6 scroll-mt-20 text-xl font-semibold tracking-tight text-[#0F172A] first:mt-0">{children}</h3>
  ),
  h3: ({ children }) => (
    <h4 className="mb-2 mt-5 scroll-mt-20 text-lg font-semibold text-[#0F172A] first:mt-0">{children}</h4>
  ),
  h4: ({ children }) => (
    <h5 className="mb-2 mt-4 text-base font-semibold text-slate-800 first:mt-0">{children}</h5>
  ),
  ul: ({ children }) => <ul className="mb-4 ml-6 list-disc space-y-2 text-slate-600 marker:text-[#4F46E5]">{children}</ul>,
  ol: ({ children }) => (
    <ol className="mb-4 ml-6 list-decimal space-y-2 text-slate-600 marker:text-[#4F46E5]">{children}</ol>
  ),
  li: ({ children }) => <li className="pl-1 text-sm leading-relaxed md:text-base [&>p]:mb-2 [&>p:last-child]:mb-0">{children}</li>,
  a: ({ href, children }) => (
    <a
      href={href}
      className="public-tech-link"
...
function ConferenceContactChannels() {
  ...
  return (
    <>
      <p className="mt-8 text-sm text-slate-600 md:text-base">{t("contactPageLead")}</p>
...
export function PublicStaticPage({ slug }: { slug: EditablePageSlug }) {
  ...
  return (
    <main className="container max-w-3xl py-12 md:py-16">
      <h1 className="text-2xl font-bold text-[#0F172A] md:text-3xl">{title}</h1>
      ...
      {loading ? (
        ...
        >
          <Loader2 className="h-9 w-9 animate-spin text-[#4F46E5]" aria-hidden />
        </div>
      ) : showPlaceholder ? (
        <p className="mt-6 text-slate-600">{t("infoPlaceholderBody")}</p>
      ) : (
        <div className="static-page-markdown mt-6 text-sm text-slate-600 md:text-base [&_a]:break-words">
